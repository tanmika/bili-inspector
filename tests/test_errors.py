...
def test_error_envelope_shape():
    ctx = CommandContext(command="subtitles", bvid="BV1xxxxxxxxx", keyword=None, page=None, limit=None, session_name="main", out_dir=None, json_output=True, verbose=False)
    envelope = error_envelope(ctx, SubtitleLangNotFoundError(["ai-en"]))
    data = envelope.to_dict()
    assert data["ok"] is False
    assert data["command"] == "subtitles"
    assert data["input"]["bvid"] == "BV1xxxxxxxxx"
